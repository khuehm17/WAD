...
void FXOS8700CQ_Accel_Calib()
{
    ...
    Xout_Accel_14_bit = ((int16_t) (AccelData[0]<<8 | AccelData[1])) >> 2;		// Compute 14-bit X-axis acceleration output value
    Yout_Accel_14_bit = ((int16_t) (AccelData[2]<<8 | AccelData[3])) >> 2;		// Compute 14-bit Y-axis acceleration output value
    Zout_Accel_14_bit = ((int16_t) (AccelData[4]<<8 | AccelData[5])) >> 2;		// Compute 14-bit Z-axis acceleration output value
...
void FXOS8700CQ_Magnet_Calib()
{
	...
	while (i < 150)		// This will take ~30s (94 samples * 1/3.125)
	{
        ...
        Xout_Mag_16_bit = (short) (MagData[0]<<8 | MagData[1]);		// Compute 16-bit X-axis magnetic output value
        Yout_Mag_16_bit = (short) (MagData[2]<<8 | MagData[3]);		// Compute 16-bit Y-axis magnetic output value
        Zout_Mag_16_bit = (short) (MagData[4]<<8 | MagData[5]);		// Compute 16-bit Z-axis magnetic output value
